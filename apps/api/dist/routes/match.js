...
router.get("/pending", async (req, res) => {
    try {
        const pendingMatches = await prismaClient_1.prisma.match.findMany({
            where: {
                courtId: null,
                winnerId: null
            },
...
router.post("/assign", async (req, res) => {
    try {
        ...
        // 检查比赛是否存在且未分配
        const match = await prismaClient_1.prisma.match.findUnique({
            where: { id: matchId },
        ...
        // 检查场地是否存在且可用
        const court = await prismaClient_1.prisma.court.findUnique({
            where: { id: courtId },
            include: {
                matches: {
                    where: {
        ...
        // 分配比赛到场地
        const updatedMatch = await prismaClient_1.prisma.match.update({
            where: { id: matchId },
