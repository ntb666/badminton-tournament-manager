...
// 配置文件路径
const CONFIG_FILE = path_1.default.join(__dirname, '../config/system.json');
// 确保配置目录存在
...
router.get("/export", async (req, res) => {
    try {
        ...
        // 按比赛类型统计
        const typeStats = {
            MEN_DOUBLE: teams.filter(t => t.type === client_1.MatchType.MEN_DOUBLE).length,
            WOMEN_DOUBLE: teams.filter(t => t.type === client_1.MatchType.WOMEN_DOUBLE).length,
            MIX_DOUBLE: teams.filter(t => t.type === client_1.MatchType.MIX_DOUBLE).length
        };
        const exportData = {
            metadata: {
 ...
 * 如果数据库中没有场地，根据配置自动创建
 */
async function ensureCourtsInitialized(courtCount) {
    try {
 ...
 */
async function updateCourtsInDatabase(targetCourtCount) {
    try {
 ...
 * 重新整理场地名称，确保编号连续
 */
async function reorderCourtNames(targetCount) {
    try {
...
router.get("/database", async (req, res) => {
    try {
        ...
        const statistics = {
            ...
            courts: {
                total: courts.length,
                withMatches: matches.filter((m) => m.courtId !== null && m.status !== 'completed').length
            },
            matches: {
                total: matches.length,
                byStatus: {
                    pending: matches.filter((m) => m.status === 'pending').length,
                    assigned: matches.filter((m) => m.status === 'assigned').length,
                    playing: matches.filter((m) => m.status === 'playing').length,
                    completed: matches.filter((m) => m.status === 'completed').length
                },
            ...
            tournaments: {
                ...
                byType: {
                    MEN_DOUBLE: tournaments.filter((t) => t.matchType === 'MEN_DOUBLE').length,
                    WOMEN_DOUBLE: tournaments.filter((t) => t.matchType === 'WOMEN_DOUBLE').length,
                    MIX_DOUBLE: tournaments.filter((t) => t.matchType === 'MIX_DOUBLE').length
                }
            },
            tournamentRounds: {
                total: tournamentRounds.length,
                byStatus: {
                    pending: tournamentRounds.filter((r) => r.status === 'pending').length,
                    active: tournamentRounds.filter((r) => r.status === 'active').length,
                    completed: tournamentRounds.filter((r) => r.status === 'completed').length
                }
            },
            tournamentTeams: {
                total: tournamentTeams.length,
                byStatus: {
                    active: tournamentTeams.filter((t) => t.status === 'active').length,
                    eliminated: tournamentTeams.filter((t) => t.status === 'eliminated').length,
                    withdrawn: tournamentTeams.filter((t) => t.status === 'withdrawn').length
                }
            }
        };
