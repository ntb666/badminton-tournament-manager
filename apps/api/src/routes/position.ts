...
// 计算比赛位置的函数
const calculateAndStorePositions = async (matchType?: string) => {
  try {
    // 构建查询条件
    const whereCondition: any = {};
    if (matchType) {
      whereCondition.matchType = matchType as any;
    }
    ...
    groupedMatches.forEach(round => {
      round.sort((a: any, b: any) => a.id - b.id);
    });

    const positions: Record<string, [number, boolean]> = {}; // key: matchId, value: [topPosition, isLong]
    ...
    console.log('分组比赛数据:', groupedMatches.map((round, index) => ({
      ...
      matches: round.map((m: any) => ({ id: m.id, parentId: m.parentId }))
    })));
    
    // 先找出第二轮中待定的比赛
    const secondRoundOnePendingMatches: any[] = [];
    const secondRoundBothPendingMatches: any[] = [];

    if (groupedMatches[1]) {
      groupedMatches[1].forEach((match: any) => {
    ...
    console.log('🔍 第二轮中有待定队伍的比赛:', secondRoundOnePendingMatches.map((m: any) => ({
      id: m.id,
    ...
    // 获取第一轮中将要显示的比赛
    const firstRoundVisibleMatches: any[] = [];
    if (groupedMatches[0]) {
      groupedMatches[0].forEach((match: any) => {
        const teamADisplayName = getTeamDisplayName(match.teamA);
        const teamBDisplayName = getTeamDisplayName(match.teamB);
        if (teamADisplayName !== '待定' && teamBDisplayName !== '待定') {
    ...
    console.log('🔍 第一轮中将要显示的比赛:', firstRoundVisibleMatches.map((m: any) => ({
      id: m.id,
      teamA: getTeamDisplayName(m.teamA),
      teamB: getTeamDisplayName(m.teamB)
    })));
    ...
    for (let roundIndex = 0; roundIndex < groupedMatches.length; roundIndex++) {
      ...
      for (let matchIndex = 0; matchIndex < round.length; matchIndex++) {
        ...
        if (roundIndex === 0) {
          ...
          if (teamADisplayName !== '待定' && teamBDisplayName !== '待定') {
            const visibleIndex = firstRoundVisibleMatches.findIndex((m: any) => m.id === match.id);
            if (visibleIndex >= 0) {
              const parentMatch = groupedMatches[1]?.find((m: any) => m.id === match.parentId);
              if (parentMatch && groupedMatches[1]) {
                if (secondRoundOnePendingMatches.some((m: any) => m.id === parentMatch.id)) {
                  // 父比赛在 secondRoundOnePendingMatches 中 - 长卡片
                  const parentIndex = groupedMatches[1].findIndex((m: any) => m.id === parentMatch.id);
                ...
                } else if (secondRoundBothPendingMatches.some((m: any) => m.id === parentMatch.id)) {
...
router.get('/list', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      positions: matches.reduce((acc: any, match) => {
